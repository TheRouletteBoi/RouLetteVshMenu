    ...
    static void Initialize()
    {
        ...
        switch (g_FindActiveGame.m_MenuToLoad)
        {
            case FindActiveGame::PatchedMenu::BO2Paradox:
            {
                ...
                {
                    SetMem<uint32_t>(Ida2Mem(sprxInternalName, 0xADB8), 0x38600001);            // read license key return true

                    uint32_t license_and_auth_vars[] = { 0x1CC1D99A, 0xD0025C50, 0x001337EA, 0x01000000 };
                    WriteProcessMemory(g_FindActiveGame.GetRunningGameProcessId(), (void*)Ida2Mem(sprxInternalName, 0x50E40), &license_and_auth_vars, sizeof(license_and_auth_vars));

                    SetNop(Ida2Mem(sprxInternalName, 0xAF5C));                                  // stop connection to paradox.sh
                    ...
                    uint32_t malloc_0x70_address = GetMem<uint32_t>(Ida2Mem(sprxInternalName, 0x40094));
                    ...
                    WriteProcessMemory(g_FindActiveGame.GetRunningGameProcessId(), (void*)Ida2Mem(sprxInternalName, 0x60EF0), &server_data, sizeof(server_data));

                    SetNop(Ida2Mem(sprxInternalName, 0xBB2C));                                  // update module
                    SetBranch(Ida2Mem(sprxInternalName, 0xADC4), Ida2Mem(sprxInternalName, 0xAEEC));                    // skip the bullshit
                    SetBranch(Ida2Mem(sprxInternalName, 0xAF60), Ida2Mem(sprxInternalName, 0xB800));                    // skip the bullshit
                    SetBranch(Ida2Mem(sprxInternalName, 0xAB04), Ida2Mem(sprxInternalName, 0xAD40));                    // skip the bullshit
                    SetBranch(Ida2Mem(sprxInternalName, 0x3525C), Ida2Mem(sprxInternalName, 0x355B0));                    // skip everything except for malloc
                    SetMem<uint32_t>(0x10021594, 0xDE7BBE46);           // xor value 2
                    SetBranch(Ida2Mem(sprxInternalName, 0x2DBAC), Ida2Mem(sprxInternalName, 0x2DC24));                    // skip thread exit. ##Crashes sometimes if load too quickly or too slowly
            ...
            case FindActiveGame::PatchedMenu::BO2Kebab:
            {
                const char* moduleName = "BO2_Retro";
                if (IsModuleLoaded(moduleName))
                {
                    uint8_t hookAddresses[] = {
                        
                    };

                    uint8_t crackedByStr[] = {
                    ...
                    vsh::ShowNotificationWithIcon(L"Kebab 1.5.3 has been patched", vsh::NotifyIcon::Pen, vsh::NotifySound::Trophy);
                }
                else 
                    vsh::ShowNotificationWithIcon(L"Kebab sprx not detected", vsh::NotifyIcon::Caution, vsh::NotifySound::Trophy);
                break;
            }
        }
    }
